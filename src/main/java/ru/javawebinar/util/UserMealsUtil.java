...
import ru.javawebinar.model.UserMealWithExceed;
...
//        .toLocalTime();
    ...
    public static List<UserMealWithExceed> getFilteredWithExceeded(List<UserMeal> mealList, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        Map<UserMeal, LocalDate> result = new TreeMap<UserMeal,LocalDate>();
        for (UserMeal userMeal : mealList) {
            if (userMeal.getDateTime().toLocalTime().isAfter(startTime) && userMeal.getDateTime().toLocalTime().isBefore(endTime)) {
                result.put(userMeal, userMeal.getDateTime().toLocalDate());
            }
        }
